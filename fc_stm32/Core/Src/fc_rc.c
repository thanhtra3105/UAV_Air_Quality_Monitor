...
void RC_Process(VehicleState_t *veh) {
    ...
    if (NRF24_Available()) {
        len_nrf = NRF24_GetDynamicPayloadSize();
        Serial_printf(&huart1, "%d\r\n", len_nrf);
        if (len_nrf == sizeof(ControlData)) {
            NRF24_Read((uint8_t*)&veh->rc_raw, len_nrf);

            veh->throttle = veh->rc_raw.chinhtocdoquat;
            bool poshold_active = veh->rc_raw.nut1 && veh->rc_raw.nut2;
        ...
        // Pack telemetry response for ACK payload
