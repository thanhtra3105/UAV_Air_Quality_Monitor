            ...
            if ser.in_waiting > 0:
                ...
                if line:
                    try:
                        ...
                        state["sensors"]["pm25"] = data.get("pm25", state["sensors"]["pm25"]) * 1000
...
def sensor_reader():
    """Thread đọc cảm biến liên tục (giống code mẫu nhưng lưu vào state và CSV)"""
    global csv_samples_collected, csv_logging_active
    
    while True:
        if time.time() - last_uart_update > UART_TIMEOUT:
            state["battery"] = 0
        ...
        if bus is not None:
            ...
            if sensor_data:
                ...
                # Ghi vào CSV nếu đang active và chưa đủ 30 mẫu
                if csv_logging_active and csv_samples_collected < csv_max_samples:
                    ...
                    # Tự động dừng khi đủ 30 mẫu
                    if csv_samples_collected >= csv_max_samples:
                        stop_csv_logging()
                
            else:
                # Fallback random nếu đọc lỗi
                fallback_data = {
                    'tvoc': random.uniform(0, 600),
                    'eco2': random.uniform(400, 2000),
                    'temp': random.uniform(20, 35),
                    'hum': random.uniform(40, 85),
                    'pm25': state['sensors']['pm25'] if state['sensors']['pm25'] else random.uniform(10, 150),
                    'co': state['sensors'].get('co'),
                    'no2': state['sensors'].get('no2'),
                }
                fallback_aqi, fallback_main, fallback_components = calculate_vietnam_aqi(
                    pm25=fallback_data['pm25'], co=fallback_data.get('co'), no2=fallback_data.get('no2'),
                    eco2=fallback_data['eco2'], tvoc=fallback_data['tvoc'],
                    temp=fallback_data['temp'], hum=fallback_data['hum']
                )
                fallback_data['aqi'] = fallback_aqi
                fallback_data['aqi_category'] = get_aqi_category(fallback_aqi)
                state["sensors"]["eco2"] = fallback_data['eco2']
                state["sensors"]["tvoc"] = fallback_data['tvoc']
                state["sensors"]["temp"] = fallback_data['temp']
                state["sensors"]["hum"] = fallback_data['hum']
                state["sensors"]["aqi"] = fallback_data['aqi']
                state["sensors"]["aqi_category"] = fallback_data['aqi_category']
                state["sensors"]["aqi_main_pollutant"] = fallback_main
                state["sensors"]["aqi_components"] = fallback_components
                
                # Ghi fallback data vào CSV nếu đang active
                if csv_logging_active and csv_samples_collected < csv_max_samples:
                    csv_samples_collected += 1
                    append_to_csv(csv_samples_collected, csv_position, fallback_data)
                    print(f"[CSV] ⚠️ Saved fallback sample {csv_samples_collected}/{csv_max_samples}")
                    
                    if csv_samples_collected >= csv_max_samples:
                        stop_csv_logging()
        else:
            # Fallback khi không có I2C
            fallback_data = {
                'tvoc': random.uniform(0, 600),
                'eco2': random.uniform(400, 2000),
                'temp': random.uniform(20, 35),
...
# ==================== START THREADS ====================
# Start UART thread
uart_thread = threading.Thread(target=uart_reader_thread, daemon=True)
uart_thread.start()
